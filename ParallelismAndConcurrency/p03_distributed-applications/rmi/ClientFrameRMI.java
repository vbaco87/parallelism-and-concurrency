...
public class ClientFrameRMI extends JFrame implements ActionListener {
	...
	protected void btnGeographyactionPerformed(ActionEvent arg0) {
		...
		try {
			...
			question = trivialSolitaireService.next(id, "GEO");
			getQuestionsActions();
	...
	protected void btnScienceactionPerformed(ActionEvent arg0) {
		...
		try {
			...
			question = trivialSolitaireService.next(id, "SCIENCE");
			getQuestionsActions();
			displayQuestion(question);
	...
	protected void btnArtactionPerformed(ActionEvent arg0) {
		...
		try {
			...
			question = trivialSolitaireService.next(id, "ART");
			getQuestionsActions();
			displayQuestion(question);
	...
	private TrivialSolitaire trivialSolitaireService;
	private int id;
	...
	/*
	 * COMPLETE add here other non-GUI attributes and helper methods
	 */

	private void displayQuestion(Question questionToAsk) {
	...
	private void connect() throws Exception {
		...
		trivialSolitaireService = (TrivialSolitaire) registry.lookup("TrivialSolitaire");
		id = trivialSolitaireService.Hello();
		JOptionPane.showMessageDialog(connectionFrame, "Your player ID is: " + id);
	}

	private void disconnect() {
		try {
			trivialSolitaireService.stop(id);
